...
void
typedecl::generate_all_symmetry_decl(symmetryclass & symmetry)
{
  if (next != NULL) {
...
/* AP: code for generate_scalarset_list in realtypedecl */
charlist *realtypedecl::generate_scalarset_list(charlist * sl)
{
  return sl;
}

charlist *enumtypedecl::generate_scalarset_list(charlist * sl)
{
  return sl;
}

charlist *subrangetypedecl::generate_scalarset_list(charlist * sl)
{
  return sl;
}
...
charlist *scalarsettypedecl::generate_scalarset_list(charlist * sl)
{
  if (getsize() > 1) {
    if (sl == NULL)
      return new charlist((char *) "", this, NULL);
    else {
      for (charlist * sv = sl; sv != NULL; sv = sv->next)
        if (sv->e == this)
          return sl;
      return new charlist((char *) "", this, sl);
    }
  } else
    return sl;
}
...
/* AP: code for generate_permute_function in realtypedecl */
void realtypedecl::generate_permute_function()
{
  if (already_generated_permute_function)
    return;
  else
    already_generated_permute_function = TRUE;

  fprintf(codefile, "void %s::Permute(PermSet& Perm, int i) {};\n",
          mu_name);
};
...
void arraytypedecl::generate_permute_function()
{
  ...
  if (indextype->gettypeclass() == typedecl::Scalarset
    ...
    if (elementtype->issimple())
      fprintf(codefile, "  temp = *this;\n" "  for (j=%d; j<=%d; j++)\n" "     (*this)[j].value(temp[Perm.revperm_%s[Perm.in_%s[i]][j-%d]].value());", indextype->getleft(), indextype->getright(),	// j-for
              indextype->mu_name, indextype->mu_name, indextype->getleft()
             );
    else
      fprintf(codefile, "  temp = *this;\n" "  for (j=%d; j<=%d; j++)\n" "    (*this)[j] = temp[Perm.revperm_%s[Perm.in_%s[i]][j-%d]];", indextype->getleft(), indextype->getright(),	// j-for
              indextype->mu_name, indextype->mu_name, indextype->getleft()
             );
  }
...
void arraytypedecl::generate_simple_sort()
{
  ...
  if (indextype->gettypeclass() == typedecl::Scalarset
      && indextype->getsize() > 1)
    generate_simple_sort(indextype->getsize(), indextype->mu_name,
                         indextype->getleft());
...
void arraytypedecl::generate_simple_sort(int size, char *name, int left)
{
  ...
  if (elementtype->issimple()) {
    ...
    fprintf(codefile, "          if (j==count_%s)\n" "            {\n" "                value[j].value((*this)[i+%d].value());\n" "              for (z=0; z<%d; z++)\n" "                pos_%s[j][z] = FALSE;\n" "              pos_%s[j][i] = TRUE;\n" "              count_%s++;\n" "            }\n" "        }\n" "    }\n", name,	// count_%s
            left, size,		// z-for
            name,		// pos_%s
            name,		// pos_%s
            name		// count_%s
...
void arraytypedecl::generate_limit2(char *var, typedecl * e)
{
  ...
  if (indextype->gettypeclass() == typedecl::Scalarset
      && indextype->getsize() > 1)
    generate_limit2(indextype->getsize(), indextype->mu_name, var,
                    indextype->getleft());
  else if (indextype->gettypeclass() == typedecl::Union
           && ((uniontypedecl *) indextype)->IsUnionWithScalarset()) {
    for (t = ((uniontypedecl *) indextype)->getunionmembers(); t != NULL;
         t = t->next) {
      d = (typedecl *) t->s->getvalue();
      if (d->gettypeclass() == typedecl::Scalarset && d->getsize() > 1)
...
void arraytypedecl::generate_limit2(int size, char *name, char *var,
                                    int left)
{
  // please make sure that i,j,k can be modified
...
void arraytypedecl::generate_limit_step1(int limit_strategy)
{
  ...
  if (indextype->gettypeclass() == typedecl::Scalarset
      && indextype->getsize() > 1)
    generate_limit_step2(indextype, (char *) "", elementtype, 0,
                         limit_strategy);
  else if (indextype->gettypeclass() == typedecl::Union
           && ((uniontypedecl *) indextype)->IsUnionWithScalarset()) {
    for (t = ((uniontypedecl *) indextype)->getunionmembers(); t != NULL;
      ...
      d = (typedecl *) t->s->getvalue();
      if (d->gettypeclass() == typedecl::Scalarset && d->getsize() > 1)
        generate_limit_step2(d, (char *) "", elementtype, 0,
                             limit_strategy);
    }
  }
}

void arraytypedecl::generate_limit_step2(typedecl * d, char *var,
    typedecl * e, int local_num,
    int limit_strategy)
{
  stelist *t;
  ste *f;
  ...
  if (e->gettypeclass() == typedecl::Scalarset && e->getsize() > 1)
    ...
    switch (e2->gettypeclass()) {
      case typedecl::Scalarset:
        if (e2->getsize() == 1)
          generate_limit_step2(d,
                               tsprintf("%s[mu_%s]", var,
                                        ((scalarsettypedecl *)
                                         e2)->getidvalues()->
                                        getname()->getname()),
                               e->getelementtype(), local_num,
                               limit_strategy);
        break;
      case typedecl::Union:
          ...
          switch (d2->gettypeclass()) {
            ...
            case typedecl::Range:
              fprintf(codefile,
                      ...
                      "  for (i%d = %d; i%d <= %d; i%d++)\n"
                      "  {\n",
                      local_num, d2->getleft(), local_num, d2->getright(),
                      local_num);
              generate_limit_step2(d, tsprintf("%s[i%d]", var, local_num),
                                   e->getelementtype(), local_num + 1,
                                   limit_strategy);
              fprintf(codefile, "  }\n");
              break;
          }
      ...
      case typedecl::Range:
        fprintf(codefile,
                ...
                "  for (i%d = %d; i%d <= %d; i%d++)\n"
                "  {\n",
                local_num, e2->getleft(), local_num, e2->getright(),
                local_num);
        generate_limit_step2(d, tsprintf("%s[i%d]", var, local_num),
                             e->getelementtype(), local_num + 1,
                             limit_strategy);
    ...
    for (f = e->getfields(); f != NULL; f = f->next) {
      generate_limit_step2(d,
                           tsprintf("%s.mu_%s", var,
                                    f->getname()->getname()),
                           f->getvalue()->gettype(), local_num,
                           limit_strategy);
    }
  }
}

void arraytypedecl::generate_limit_step3(typedecl * d, char *var,
    typedecl * e, int limit_strategy,
    bool isunion)
{
  switch (limit_strategy) {
    ...
    case 4:
      if (e == d)
        generate_limit4(d->getsize(), d->mu_name, var, d->getleft(),
                        isunion);
      break;
    case 5:
      if (e != d && e->getsize() > 1)
        generate_limit5(d->mu_name, d->getsize(), d->getleft(), var,
                        e->mu_name, e->getsize(), e->getleft(), isunion);
      break;
  }
}
...
void arraytypedecl::generate_limit3(int size, char *name, char *var,
                                    int left)
{
  // please make sure that i,j,k can be modified
...
void arraytypedecl::generate_limit4(typedecl * d, char *var, typedecl * e)
{
  ...
  if (e->gettypeclass() == typedecl::Scalarset && e == d)
...
void arraytypedecl::generate_limit4(int size, char *name, char *var,
                                    int left, bool isunion)
{
  if (isunion)
    fprintf(codefile, "\n" "      // refinement -- graph structure for a single scalarset\n" "      //               as in array S1 of S1\n" "      // only if there is more than 1 permutation in class\n" "      if (Perm.MTO_class_%s() && count_%s<%d)\n" "        {\n" "          exists = FALSE;\n" "          split = FALSE;\n" "          for (k=0; k<%d; k++) // step through class\n" "            if (!(*this)[k+%d]%s.isundefined()\n" "                &&(*this)[k+%d]%s!=k+%d\n" "                &&(*this)[k+%d]%s>=%d\n" "                &&(*this)[k+%d]%s<=%d)\n" "              exists = TRUE;\n" "          if (exists)\n" "            {\n" "              for (i=0; i<count_%s; i++) // for value priority\n" "                {\n" "                  for (j=0; j<count_%s; j++) // for value priority\n" "                    {\n" "                      exists = FALSE;\n" "                      for (k=0; k<%d; k++) // step through class\n" "                        goodset_%s[k] = FALSE;\n" "                      for (k=0; k<%d; k++) // step through class\n" "                        if (pos_%s[i][k] \n" "                            && !(*this)[k+%d]%s.isundefined()\n" "                            && (*this)[k+%d]%s!=k+%d\n" "                            && (*this)[k+%d]%s>=%d\n"	// extra checking
            "                            && (*this)[k+%d]%s<=%d\n"	// extra checking
...
void arraytypedecl::generate_limit5(typedecl * d, char *var, typedecl * e)
{
  ...
  if (e->gettypeclass() == typedecl::Scalarset
    ...
    generate_limit5(d->mu_name, d->getsize(), d->getleft(),
                    var, e->mu_name, e->getsize(), e->getleft(), FALSE);
  else if (e->gettypeclass() == typedecl::Union) {
...
void arraytypedecl::generate_canonicalize(int size,
    ...
    int scalarsetleft)
{
  fprintf(codefile, "  if (Perm.MTO_class_%s())\n" "    {\n", name);
...
void arraytypedecl::generate_while(charlist * scalarsetlist)
{
  ...
  for (sl = scalarsetlist; sl != NULL; sl = sl->next)
    fprintf(codefile,
            ...
            sl->e->mu_name, sl->e->mu_name, sl->e->getsize()
           );

  fprintf(codefile, "  while ( while_guard )\n" "    {\n");
...
/* AP: code for generate_simple_canon_function in realtypedecl */
void realtypedecl::generate_simple_canon_function(symmetryclass & symmetry)
{
  if (already_generated_simple_canon_function)
    return;
  else
    already_generated_simple_canon_function = TRUE;
...
void subrangetypedecl::
generate_simple_canon_function(symmetryclass & symmetry)
{
  if (already_generated_simple_canon_function)
    return;
  else
...
generate_simple_canon_function(symmetryclass & symmetry)
{
  ...
  if (indextype->getstructure() != typedecl::ScalarsetVariable
    ...
    fprintf(codefile, "void %s::SimpleCanonicalize(PermSet& Perm)\n" "{\n" "  for (int j=0; j<%d; j++)\n" "    array[j].SimpleCanonicalize(Perm);\n" "}\n", mu_name,	// %s::Canonicalize
            indextype->getsize()	// j-for
           );
  else
    fprintf(codefile,
...
//                    d->mu_name, thisleft, d->mu_name, // class_%s, value-%d, class_%s// fixing union permutation bug
                d->mu_name, d->getleft(), d->mu_name, d->getsize(),	// i-for
...
/* AP: code for generate_canonicalize_function in realtypedecl */
void realtypedecl::generate_canonicalize_function(symmetryclass & symmetry)
{
  if (already_generated_canonicalize_function)
    return;
  else
    already_generated_canonicalize_function = TRUE;

  fprintf(codefile, "void %s::Canonicalize(PermSet& Perm) {};\n", mu_name);
};
...
void subrangetypedecl::
generate_canonicalize_function(symmetryclass & symmetry)
{
  if (already_generated_canonicalize_function)
    return;
  else
...
/* AP: generate_simple_limit_function */
void realtypedecl::generate_simple_limit_function(symmetryclass & symmetry)
{
  if (already_generated_simple_limit_function)
    return;
  else
    already_generated_simple_limit_function = TRUE;

  fprintf(codefile, "void %s::SimpleLimit(PermSet& Perm) {};\n", mu_name);
};
...
void subrangetypedecl::
generate_simple_limit_function(symmetryclass & symmetry)
{
  if (already_generated_simple_limit_function)
    return;
  else
...
/* AP: code for generate_array_limit_function in realtypedecl */
void realtypedecl::generate_array_limit_function(symmetryclass & symmetry)
{
  if (already_generated_array_limit_function)
    return;
  else
    already_generated_array_limit_function = TRUE;

  fprintf(codefile, "void %s::ArrayLimit(PermSet& Perm) {};\n", mu_name);
};
...
void subrangetypedecl::
generate_array_limit_function(symmetryclass & symmetry)
{
  if (already_generated_array_limit_function)
    return;
  else
...
/* AP: code for generate_limit_function in realtypedecl */
void realtypedecl::generate_limit_function(symmetryclass & symmetry)
{
  if (already_generated_limit_function)
    return;
  else
    already_generated_limit_function = TRUE;

  fprintf(codefile, "void %s::Limit(PermSet& Perm) {};\n", mu_name);
};
...
//       for (sv=scalarsetvalue; sv!=NULL; sv = sv->next)
    ...
    // even though it may have only 1 permutation
    for (sv = scalarsetlist; sv != NULL; sv = sv->next)
      fprintf(codefile, "  // initializing pos array\n" "  for (i=0; i<%d; i++)\n" "    for (j=0; j<%d; j++)\n" "      pos_%s[i][j]=FALSE;\n" "  count_%s = 0;\n" "  while (1)\n" "    {\n" "      exists = FALSE;\n" "      for (i=0; i<%d; i++)\n" "       if (Perm.class_%s[i] == count_%s)\n" "         {\n" "           pos_%s[count_%s][i]=TRUE;\n" "           exists = TRUE;\n" "         }\n" "      if (exists) count_%s++;\n" "      else break;\n" "    }\n", sv->e->getsize(), sv->e->getsize(), sv->e->mu_name,	// initialization
              sv->e->mu_name,	// count_%s
              sv->e->getsize(),	// i-for
              sv->e->mu_name, sv->e->mu_name,	// if-guard
              sv->e->mu_name, sv->e->mu_name,	// pos_%s[count%s]
...
/* AP: code for generate_multisetlimit_function in realtypedecl */
void realtypedecl::
generate_multisetlimit_function(symmetryclass & symmetry)
{
  if (already_generated_multisetlimit_function)
    return;
  else
    already_generated_multisetlimit_function = TRUE;
...
void multisettypedecl::generate_multiset_while_guard(charlist *
    scalarsetlist)
{
  charlist *sl;
...
{
  generate_limit_step2((char *) "", elementtype, 0, limit_strategy);
}

void multisettypedecl::generate_limit_step2(char *var, typedecl * e,
    int local_num,
    int limit_strategy)
{
  stelist *t;
  ...
  if (e->gettypeclass() == typedecl::Scalarset && e->getsize() > 1)
    ...
    switch (e2->gettypeclass()) {
      case typedecl::Scalarset:
        if (e2->getsize() == 1)
          generate_limit_step2(tsprintf
                               ("%s[mu_%s]", var,
                                ((scalarsettypedecl *) e2)->
                                getidvalues()->getname()->getname()),
                               e->getelementtype(), local_num,
                               limit_strategy);
        break;
      case typedecl::Union:
...
void multisettypedecl::generate_limit_step3(char *var, typedecl * e,
    int limit_strategy,
    bool isunion)
{
  if (limit_strategy == 5)
    generate_multiset_limit5(var, e->mu_name, e->getsize(), e->getleft(),
                             isunion);
}
...
void multisettypedecl::generate_multiset_limit5(char *var, char *name2,
    int size2, int left2,
    bool isunion)
{
  if (isunion)
...
generate_multisetlimit_function(symmetryclass & symmetry)
{
  ...
  if (!elementtype->HasScalarsetVariable())
    ...
    fprintf(codefile, "void %s::MultisetLimit(PermSet& Perm)\n" "{\n" "  // indexes\n" "  int i,j,k,z;\n" "  // while guard\n" "  bool while_guard, while_guard_temp;\n" "  // sorting\n" "  static %s value[%d];\n" "  // limit\n" "  bool exists;\n" "  bool split;\n", mu_name,	// %s::Canonicalize
    ...
    fprintf(codefile, "  if (current_size == 1)\n" "    {\n");
    if (elementtype->HasScalarsetVariable())
      fprintf(codefile, "      array[0].SimpleLimit(Perm);\n");
    if (elementtype->HasScalarsetArrayOfFree())
      fprintf(codefile, "      array[0].ArrayLimit(Perm);\n");
    if (elementtype->HasScalarsetArrayOfS())
      fprintf(codefile, "      array[0].Limit(Perm);\n");
    fprintf(codefile, "    }\n" "  else\n" "    {\n" "\n");
