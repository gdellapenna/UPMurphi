...
* exactly one filename ending with the .m extension,\n\
  which is considered to be a precompiled UPMurphi model file.\n");
...
int compile_pddl()
{
  ...
  if (!args->force_recompile && (get_last_modified(get_pddldomainfilename())<=get_last_modified(get_mfilename()))) {
    printf("UPmurphi model is up to date\n");
    return 0;
  } else {
    printf("Compiling PDDL to UPMurphi model, please wait...\n");
    ...
    if (!error) {
      ...
      printf("UPMurphi model generated in file %s\n", get_mfilename());
...
int compile_cpp()
{
  ...
  if (!args->force_recompile && (get_last_modified(get_cppfilename())<=get_last_modified(get_execfilename()))) {
    ...
    sprintf(commandline,"%s %s %s -I\"%s\" %s -o %s",QUOTED_VALUE(COMPILER_NAME),QUOTED_VALUE(COMPILER_DEFINES),get_cppfilename(),base_include_path,QUOTED_VALUE(COMPILER_SWITCHES),get_execfilename());
    int error = system(commandline);
    if (!error) {
